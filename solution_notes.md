...
当 $$R^2 < 0.9$$ 时扩展为（约束 $$k_i \geq 0$$，用 `lsq_linear` TRF）：
...
9. **电耗模型 $$k_i \geq 0$$**：`lsq_linear` TRF 约束非负，去掉产生负二次项的线性项
...
$$R^2 < 0.9$$，扩展为 $$P = \sum k_i U_i^2 + c$$（约束 $$k_i \geq 0$$），$$R^2 = 0.9349$$。
...
|------|--------|------|------|------|------|
| 0 | 1309 | 44.34 | 119.5 | 1760.8 | 10.0 |
| 1 | 2184 | 26.60 | 131.4 | 1614.6 | 10.0 |
| 2 | 1009 | 43.77 | 131.4 | 1645.2 | 10.0 |
| 3 | 1583 | 42.93 | 119.9 | 1643.0 | 10.0 |
| 4 | 2360 | 40.19 | 130.0 | 1745.5 | 10.0 |
| 5 | 1635 | 26.01 | 120.8 | 1654.7 | 10.0 |
...
电耗模型 $$P=\sum k_i U_i^2 + c$$ 不含振打，$$E_{T_i}^{P}\approx 0$$；双向偏离模型下 $$E_{T_i}^{C}\neq 0$$，性价比 $$|E^C/E^P|\to\infty$$，振打视为"免费"降排放手段。

**差异原因**：高浓度工况需更高电压承担除尘负荷；振打能降排放但不增电耗（振打电机功耗可忽略），应优先调整。
...
低浓度工况(工况1)增幅最小（$$3.34\%$$），高浓度及大流量工况增幅较大（$$6\text{-}8\%$$）。
...
6. **振打双向偏离**：$$T_i$$ 偏离 $$T_{ref,i}$$ 双向都使效率下降（过长积灰、过频扬尘），$$r=0.5$$ 固定（限幅致不可辨识）
