...
def df_power():
    ...
    c_true = 800.0
    P = sum(k_true[i] * U[:, i] ** 2 for i in range(4)) + c_true + np.random.normal(0, 5, n)
    df = pd.DataFrame({
...
def test_predict_extended_formula(df_power):
    ...
    P = predict_power(model, U)
    expected = sum(model["k_ext"][i] * U[i] ** 2 for i in range(4)) + model["c_ext"]
